...
void main()
{
    ...
    if (!alphaTest(outColor.a, material.alphaTestCutoff))
        discard;
#endif
    if (/*depthPass*/dot(scene.lightViewPos.xyz, scene.lightViewPos.xyz) == 0)
    {
        if (outColor.a < 0.5)
            discard;
        return;
    ...
    vec3 diffuseLight = frag_in.pointLightDiffuse + directionalDiffuse(viewNormal) * shadow(frag_in.viewPos, viewNormal);
#else
