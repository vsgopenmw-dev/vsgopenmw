...
float textureProj(vec4 shadowCoord, uint cascadeIndex)
{
    ...
    //if ( shadowCoord.z > -1.0 && shadowCoord.z < 1.0) //opengl-depth-range
    if ( shadowCoord.z > 0.0 && shadowCoord.z < 1.0) //vk-depth-range
    {
        float dist = texture(shadowMap, vec3(shadowCoord.st, cascadeIndex)).r;
