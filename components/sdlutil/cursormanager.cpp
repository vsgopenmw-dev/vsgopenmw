...
namespace
{
    ...
    SurfaceUniquePtr transform(SDL_Surface* cursorSurface, float rotDegrees, Uint8 w, Uint8 h)
    {
        ...
        SDL_Surface* targetSurface = SDL_CreateRGBSurface(0, w, h, 32, redMask, greenMask, blueMask, alphaMask);
...
namespace SDLUtil
{
        ...
        Uint8 hotspot_y, Uint8 w, Uint8 h)
    {
        ...
        {
            auto transformed = transform(surface, static_cast<float>(rotDegrees), w, h);
            curs = SDL_CreateColorCursor(transformed.get(), hotspot_x, hotspot_y);
        }
