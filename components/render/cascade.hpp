...
namespace Render // namespace vulkanexamples
{
     ...
     */
    inline std::vector<Cascade> cascade(
        size_t count, float near, float far, float maxZ, float cascadeSplitLambda, const vsg::Camera& cam, const vsg::vec3& lightDir)
    {
        using vec3 = vsg::dvec3;
        std::vector<Cascade> cascades(count);
        std::vector<double> cascadeSplits(count);

        double clipRange = far - near;

        double minZ = near;
        //double maxZ = near + clipRange;

        double range = maxZ - minZ;
        double ratio = maxZ / minZ;
        ...
        for (uint32_t i = 0; i < count; ++i)
        {
            ...
            std::array<vec3, 8> frustumCorners = { vec3(-1.0, 1.0, nearDepth), vec3(1.0, 1.0, nearDepth),
                vec3(1.0, -1.0, nearDepth), vec3(-1.0, -1.0, nearDepth), vec3(-1.0, 1.0, farDepth),
                vec3(1.0, 1.0, farDepth), vec3(1.0, -1.0, farDepth), vec3(-1.0, -1.0, farDepth) };

            // Project frustum corners into world space
            auto invCam = vsg::inverse(cam.projectionMatrix->transform() * cam.viewMatrix->transform());
            ...
            cascades[i].lookAt
                = vsg::LookAt(frustumCenter + vec3(lightDir) * minExtents.z, frustumCenter, vec3(0.0, 1.0, 0.0));
            cascades[i].ortho = vsg::Orthographic(
                minExtents.x, maxExtents.x, minExtents.y, maxExtents.y, 0.0, maxExtents.z - minExtents.z);
