...
namespace Render
{
    ...
    void ComputeImage::clear(vsg::ref_ptr<vsg::Image> image, vsg::RecordTraversal& visitor) const
    {
        ...
        vkCmdClearColorImage(*visitor.getCommandBuffer(), image->vk(visitor.deviceID()),
            VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, &mClearColor, static_cast<uint32_t>(1),
            &mPreClearBarrier->imageMemoryBarriers[0]->subresourceRange);
    ...
    void ComputeImage::compute(
        vsg::RecordTraversal& visitor, vsg::ref_ptr<vsg::Image> image, uint32_t groupCountX, uint32_t groupCountY) const
    {
