...
namespace Resource
{
    ...
    public:
        ...
        vsg::ref_ptr<const BulletShape> getShape(const std::string& name);
        ...
        vsg::ref_ptr<BulletShapeInstance> cacheInstance(const std::string& name);

        /// @note May return a null pointer if the object has no shape.
        vsg::ref_ptr<BulletShapeInstance> getInstance(const std::string& name);
    ...
    private:
        vsg::ref_ptr<BulletShapeInstance> createInstance(const std::string& name);

        vsg::ref_ptr<MultiObjectCache> mInstanceCache;
