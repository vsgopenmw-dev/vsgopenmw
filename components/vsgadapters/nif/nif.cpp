...
namespace vsgAdapters
{
    ...
    public:
        ...
        {
            if (!nifNode.trafo.isIdentity() || !this->mCanOptimize || !nifNode.controller.empty()
            ...
            vsg::ref_ptr<vsg::vec4Array> weights)
        {
            ...
            for (unsigned char i = 0; i < skin.bones.size(); ++i)
            {
                ...
                vsg::sphere bounds(toVsg(boneData.boundSphereCenter), boneData.boundSphereRadius);
        ...
        vsg::ref_ptr<vsg::Node> createParticleStage(Pipeline::ParticleStage stage, const vsg::Descriptors& descriptors,
            int bin, vsg::ref_ptr<vsg::Node> dispatch)
        {
            auto sg = vsg::StateGroup::create();
            auto bindComputePipeline = mBuilder.particle->getOrCreate(stage);
            sg->stateCommands = { bindComputePipeline,
                vsg::BindDescriptorSet::create(VK_PIPELINE_BIND_POINT_COMPUTE, bindComputePipeline->pipeline->layout,
                    Pipeline::COMPUTE_SET, descriptors) };
            sg->children = { dispatch };
            sg->prototypeArrayState = sNullArrayState;
            return vsgUtil::SetBin::create(bin, sg);
        }

        vsg::ref_ptr<vsg::Node> handleParticleGrowFade(
            const Nif::NiParticleGrowFade& gf, vsg::ref_ptr<vsg::Node> dispatch, float defaultSize)
        {
            auto argsData = vsg::ref_ptr{ new vsgUtil::Value<Pipeline::Data::SizeArgs>(
                Pipeline::Data::SizeArgs{ gf.growTime, gf.fadeTime, defaultSize }) };
            share(argsData);
            auto argsDescriptor = vsg::DescriptorBuffer::create(
                argsData, Pipeline::Descriptors::ARGS_BINDING, 0, VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER);
            share(argsDescriptor);
            auto dummyStorageArgsDescriptor = vsg::DescriptorBuffer::create(
                argsData, Pipeline::Descriptors::STORAGE_ARGS_BINDING, 0, VK_DESCRIPTOR_TYPE_STORAGE_BUFFER);
            share(dummyStorageArgsDescriptor);
            vsg::Descriptors descriptors{ argsDescriptor, dummyStorageArgsDescriptor };
            return createParticleStage(
                Pipeline::ParticleStage::Size, descriptors, computeBin /*Bin_SizeParticle*/, dispatch);
        }

        vsg::ref_ptr<vsg::Node> handleParticleColorModifier(
            const Nif::NiParticleColorModifier& cl, vsg::ref_ptr<vsg::Node> dispatch, float maxLifetime)
        {
            if (cl.data.empty())
                return {};
            auto colorCurve = createColorCurve(*cl.data.getPtr(), maxLifetime, COLOR_CURVE_STEP);
            auto dummyArgsData
                = vsg::ref_ptr{ new vsgUtil::Value<Pipeline::Data::EmitArgs>(Pipeline::Data::EmitArgs{}) };
            share(dummyArgsData);
            auto dummyArgsDescriptor = vsg::DescriptorBuffer::create(
                dummyArgsData, Pipeline::Descriptors::ARGS_BINDING, 0, VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER);
            share(dummyArgsDescriptor);
            vsg::Descriptors descriptors{ dummyArgsDescriptor,
                vsg::DescriptorBuffer::create(
                    colorCurve, Pipeline::Descriptors::STORAGE_ARGS_BINDING, 0, VK_DESCRIPTOR_TYPE_STORAGE_BUFFER) };
            return createParticleStage(
                Pipeline::ParticleStage::Color, descriptors, computeBin /*Bin_ColorParticle*/, dispatch);
        }
            ...
            const Nif::NiParticles& particles, const Nif::NiParticleSystemController& partctrl)
        {
            ...
            computeGroup->stateCommands = { vsg::BindDescriptorSet::create(VK_PIPELINE_BIND_POINT_COMPUTE,
                mBuilder.particle->getLayout(), Pipeline::TEXTURE_SET, vsg::Descriptors{ particlesDescriptor }) };
            ...
            for (; !affectors.empty(); affectors = affectors->next)
            {
                ...
                if (affectors->recType == Nif::RC_NiParticleGrowFade)
                    affector = handleParticleGrowFade(
                        static_cast<const Nif::NiParticleGrowFade&>(*affectors.getPtr()), dispatch, partctrl.size);
                    ...
                    affector = handleParticleColorModifier(
                        static_cast<const Nif::NiParticleColorModifier&>(*affectors.getPtr()), dispatch, maxLifetime);
            ...
            */
            ...
            auto frameArgsData = vsg::ref_ptr{ new vsgUtil::Value<Pipeline::Data::FrameArgs>() };
            auto frameArgsDescriptor = vsg::DescriptorBuffer::create(
                frameArgsData, Pipeline::Descriptors::STORAGE_ARGS_BINDING, 0, VK_DESCRIPTOR_TYPE_STORAGE_BUFFER);
            {
                auto emitArgs = vsg::ref_ptr{ new vsgUtil::Value<Pipeline::Data::EmitArgs>(emitterData) };
                share(emitArgs);
                auto argsDescriptor = vsg::DescriptorBuffer::create(
                    emitArgs, Pipeline::Descriptors::ARGS_BINDING, 0, VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER);
                share(argsDescriptor);
                vsg::Descriptors descriptors{ argsDescriptor, frameArgsDescriptor };
                computeGroup->addChild(
                    createParticleStage(Pipeline::ParticleStage::Simulate, descriptors, computeBin, dispatch));
            }
            ...
            // group->addChild(
            auto drawState = createStateGroup(draw);
            drawState->prototypeArrayState = sNullArrayState;
            sw->children = { { vsg::MASK_ALL, vsgUtil::SetBin::create(depthSortedBin, drawState) },
        ...
        void handleFlipController(const Nif::NiFlipController& flipctrl)
        {
            ...
            ctrl->index = new FlipChannel(flipctrl.mDelta, flipctrl.mSources.size());
        ...
        vsg::ref_ptr<vsg::Data> createMaterialData()
        {
            ...
            if (options.material && !Pipeline::Material::isDefault(*options.material))
                data = new MaterialValue(*options.material);
            else
                data = new MaterialValue(Pipeline::Material::createDefault());
        ...
        void handleUVController(const Nif::NiUVController& uvctrl)
        {
            ...
            auto matrix = vsgUtil::Value<vsg::mat4>::create();
        ...
        void handleVisController(const Nif::NiVisController& visctrl, vsg::Switch& sw)
        {
            ...
            auto ctrl = vsg::ref_ptr{ new Anim::Switch(vsg::ref_ptr{ new VisChannel(visctrl.mData->mVis) }) };
