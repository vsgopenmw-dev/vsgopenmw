...
namespace vsgAdapters::mygui
{
    ...
    void Texture::createManual(int width, int height, MyGUI::TextureUsage usage, MyGUI::PixelFormat format)
    {
        ...
        if (mFormat == MyGUI::PixelFormat::R8G8B8)
            ...
            mLockedImage = vsg::ubvec4Array2D::create(mWidth, mHeight, vsg::Data::Properties{ VK_FORMAT_R8G8B8A8_UNORM });
    ...
    void Texture::loadFromFile(const std::string& fname)
    {
        ...
        mWidth = textureData->width() * textureData->properties.blockWidth;
        mHeight = textureData->height() * textureData->properties.blockHeight;
