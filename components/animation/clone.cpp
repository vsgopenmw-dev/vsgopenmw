...
namespace Anim
{
    ...
    private:
        ...
        void apply(const vsg::floatArray& a) override { applyData(a); }
        void apply(const vsg::mat4Array& a) override { applyData(a); }
        template <class Data>
        void applyData(const Data& d)
        {
            if (!Controller::get(d))
                return;
            cloned = copy(d);
        ...
        void apply(const vsg::DescriptorBuffer& d) override
        {
            ...
            cloned = vsg::DescriptorBuffer::create(
                vsg::ref_ptr{ data }, d.dstBinding, d.dstArrayElement, d.descriptorType);
        }
