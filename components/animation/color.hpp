...
namespace Anim
{
    ...
    public:
        ...
        void apply(vsg::Data& data, float time) const
        {
            ...
            {
                vsg::vec3& vec
                    = *reinterpret_cast<vsg::vec3*>(reinterpret_cast<unsigned char*>(data.dataPointer()) + colorOffset);
                vec = color->value(time);
            }
            if (alpha)
            {
                vsg::vec4& a
                    = *reinterpret_cast<vsg::vec4*>(reinterpret_cast<unsigned char*>(data.dataPointer()) + alphaOffset);
