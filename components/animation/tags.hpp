...
namespace Anim
{
    ...
    public:
        ...
        auto begin() const noexcept { return mTags.begin(); }
        auto end() const noexcept { return mTags.end(); }
        auto rbegin() const noexcept { return mTags.rbegin(); }
        auto rend() const noexcept { return mTags.rend(); }
        auto lowerBound(float time) const { return mTags.lower_bound(time); }
        auto upperBound(float time) const { return mTags.upper_bound(time); }
