...
namespace Anim
{
    ...
    {
        using value_type = T;
        inline T access(const T& t) const { return t; }
    ...
    public:
        ...
        T value(float time) const override { return t_interpolate(keys, *this, time); }
        inline T operator()(const T& a, const T& b, float fraction) const { return fraction > 0.5f ? b : a; }
    };
    template <typename T>
    ...
    public:
        std::map<float, T> keys;
        T value(float time) const override { return t_interpolate(keys, *this, time); }
        inline T operator()(const T& a, const T& b, float fraction) const { return vsg::mix(a, b, fraction); }
    };
    template <typename T>
    ...
    public:
        ...
        T value(float time) const override { return t_interpolate(keys, *this, time); }
        inline T access(const Data& data) const { return data.value; }
    ...
    public:
        ...
        vsg::quat value(float time) const override { return t_interpolate(keys, *this, time); }
