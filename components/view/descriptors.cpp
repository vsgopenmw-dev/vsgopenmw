...
namespace View
{
    ...
    vsg::ref_ptr<vsg::Descriptor> dummyShadowMap()
    {
        ...
        image->properties.imageViewType = VK_IMAGE_VIEW_TYPE_2D_ARRAY;
        return vsg::DescriptorImage::create(sampler, image, Pipeline::Descriptors::VIEW_SHADOW_MAP_BINDING, 0,
    ...
    vsg::Descriptors createLightingDescriptors(vsg::ref_ptr<vsg::Data> lightData, bool compute)
    {
        ...
        vsg::Descriptors descriptors{ lightDescriptor, lightGrid, lightIndices };

        if (compute)
            descriptors.emplace_back(vsg::DescriptorBuffer::create(vsg::uivec4Value::create(vsg::uivec4(gridSize, 0)),
                Pipeline::Descriptors::VIEW_LIGHT_GRID_SIZE_BINDING, 0, VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER));
        return descriptors;
    }
}
