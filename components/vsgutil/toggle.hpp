...
namespace vsgUtil
{
    ...
    public:
        ...
        void setEnabled(bool enabled) { mSwitch->children[mIndex].mask = enabled ? enabledMask : vsg::MASK_OFF; }
        T& operator*() { return *mNode; }
        const T& operator*() const { return *mNode; }
        T* operator->() { return mNode.get(); }
        const T* operator->() const { return mNode.get(); }
    };
}
