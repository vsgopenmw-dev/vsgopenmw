...
#include <files/shaders/terrain/bindings.glsl>
    ...
    {
        vsg::ShaderStages shaders = { vsgUtil::readShader("terrain/main.vert", mShaderOptions),
            vsgUtil::readShader("terrain/main.frag", mShaderOptions) };

        shaders[1] = vsg::ShaderStage::create(*shaders[1]);
        shaders[1]->specializationConstants
            = { { 0, vsg::intValue::create(key.layerCount) } };
        ...
        vsg::DescriptorSetLayoutBindings descriptorBindings{
            ...
            { Pipeline::Descriptors::LAYER_BINDING, VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER, key.layerCount, VK_SHADER_STAGE_FRAGMENT_BIT, nullptr },
            { Pipeline::Descriptors::CHUNK_BINDING, VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER, 1, VK_SHADER_STAGE_VERTEX_BIT, nullptr }
        };
        ...
        vsg::VertexInputState::Attributes vertexAttributeDescriptions;
        {
            vertexBindingsDescriptions.push_back(
                { vertexBindingIndex, sizeof(float), VK_VERTEX_INPUT_RATE_VERTEX });
            vertexAttributeDescriptions.push_back(
                { Attributes::VERTEX_LOCATION, vertexBindingIndex++, VK_FORMAT_R32_SFLOAT, 0 });
        }
        {
            vertexBindingsDescriptions.push_back(
                { vertexBindingIndex, sizeof(vsg::vec3), VK_VERTEX_INPUT_RATE_VERTEX });
            vertexAttributeDescriptions.push_back(
                { Attributes::NORMAL_LOCATION, vertexBindingIndex++, VK_FORMAT_R32G32B32_SFLOAT, 0 });
        }
        {
            vertexBindingsDescriptions.push_back(
                { vertexBindingIndex, sizeof(vsg::ubvec4), VK_VERTEX_INPUT_RATE_VERTEX });
            vertexAttributeDescriptions.push_back(
                { Attributes::COLOR_LOCATION, vertexBindingIndex++, VK_FORMAT_R8G8B8A8_UNORM, 0 });
        }
        ...
        vsg::GraphicsPipelineStates pipelineStates{ vsg::VertexInputState::create(
            ...
            vsg::InputAssemblyState::create(), vsg::RasterizationState::create(), vsg::MultisampleState::create(),
