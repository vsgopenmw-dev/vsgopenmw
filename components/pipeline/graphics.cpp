...
namespace Pipeline
{
    ...
    vsg::ref_ptr<vsg::BindGraphicsPipeline> Graphics::create(const Options& key) const
    {
        ...
        if (key.shader.hasMode(Mode::MATERIAL))
            descriptorBindings.push_back({ Descriptors::MATERIAL_BINDING, VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER, 1,
                VK_SHADER_STAGE_FRAGMENT_BIT, nullptr });
        if (key.shader.hasMode(Mode::PARTICLE))
            descriptorBindings.push_back({ Descriptors::PARTICLE_BINDING, VK_DESCRIPTOR_TYPE_STORAGE_BUFFER, 1,
                VK_SHADER_STAGE_VERTEX_BIT, nullptr });
        if (key.shader.hasMode(Mode::MORPH))
        {
            descriptorBindings.push_back({ Descriptors::MORPH_BINDING, VK_DESCRIPTOR_TYPE_STORAGE_BUFFER, 1,
                VK_SHADER_STAGE_VERTEX_BIT, nullptr });
            descriptorBindings.push_back({ Descriptors::MORPH_WEIGHTS_BINDING, VK_DESCRIPTOR_TYPE_STORAGE_BUFFER, 1,
                VK_SHADER_STAGE_VERTEX_BIT, nullptr });
        }
        if (key.shader.hasMode(Mode::SKIN))
            descriptorBindings.push_back({ Descriptors::BONE_BINDING, VK_DESCRIPTOR_TYPE_STORAGE_BUFFER, 1,
                VK_SHADER_STAGE_VERTEX_BIT, nullptr });
        if (key.shader.hasMode(Mode::DIFFUSE_MAP))
            descriptorBindings.push_back({ Descriptors::DIFFUSE_UNIT, VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER, 1,
                VK_SHADER_STAGE_FRAGMENT_BIT, nullptr });
        if (key.shader.hasMode(Mode::DARK_MAP))
            descriptorBindings.push_back({ Descriptors::DARK_UNIT, VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER, 1,
                VK_SHADER_STAGE_FRAGMENT_BIT, nullptr });
        if (key.shader.hasMode(Mode::DETAIL_MAP))
            descriptorBindings.push_back({ Descriptors::DETAIL_UNIT, VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER, 1,
                VK_SHADER_STAGE_FRAGMENT_BIT, nullptr });
        if (key.shader.hasMode(Mode::DECAL_MAP))
            descriptorBindings.push_back({ Descriptors::DECAL_UNIT, VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER, 1,
                VK_SHADER_STAGE_FRAGMENT_BIT, nullptr });
        if (key.shader.hasMode(Mode::GLOW_MAP))
            descriptorBindings.push_back({ Descriptors::GLOW_UNIT, VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER, 1,
                VK_SHADER_STAGE_FRAGMENT_BIT, nullptr });
        if (key.shader.hasMode(Mode::BUMP_MAP))
            descriptorBindings.push_back({ Descriptors::BUMP_UNIT, VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER, 1,
                VK_SHADER_STAGE_FRAGMENT_BIT, nullptr });
        if (key.shader.hasMode(Mode::ENV_MAP))
            descriptorBindings.push_back({ Descriptors::ENV_UNIT, VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER, 1,
                VK_SHADER_STAGE_FRAGMENT_BIT, nullptr });
        if (key.shader.hasMode(Mode::TEXMAT))
            descriptorBindings.push_back({ Descriptors::TEXMAT_BINDING, VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER, 1,
                VK_SHADER_STAGE_VERTEX_BIT, nullptr });
        ...
        mSharedObjects->share(pipelineLayout);
        ...
        auto bindGraphicsPipeline
            = vsg::BindGraphicsPipeline::create(vsg::GraphicsPipeline::create(pipelineLayout, shaders, pipelineStates));
