...
namespace Pipeline
{
    std::string getFilename(const ParticleStage& stage)
    {
        switch (stage)
        {
            case ParticleStage::Simulate:
                return "simulate.comp";
            case ParticleStage::Color:
                return "color.comp";
            case ParticleStage::Size:
                return "size.comp";
        }
    }
        ...
        : mShaderOptions(readShaderOptions)
    {
        ...
        vsg::DescriptorSetLayoutBindings particleDescriptorBindings{ { Descriptors::PARTICLE_BINDING,
            VK_DESCRIPTOR_TYPE_STORAGE_BUFFER, 1, computeStage, nullptr } };

        vsg::DescriptorSetLayoutBindings computeDescriptorBindings{
            { Descriptors::STORAGE_ARGS_BINDING, VK_DESCRIPTOR_TYPE_STORAGE_BUFFER, 1, computeStage, nullptr },
