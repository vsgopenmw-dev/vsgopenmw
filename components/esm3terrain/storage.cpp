...
namespace ESMTerrain
{
    ...
    void Storage::fixNormal (vsg::vec3& normal, int cellX, int cellY, int col, int row, LandCache& cache)
    {
        ...
        if (data)
        {
            normal = vsg::normalize(vsg::vec3(
                data->mNormals[col*ESM::Land::LAND_SIZE*3+row*3],
                                            ...
                                            vsg::ref_ptr<vsg::ubvec4Array> &colours)
    {
        ...
        positions = vsg::vec3Array::create(numVerts*numVerts);
                ...
                for (int col=colStart; col<colEnd; col += increment)
                {
                    ...
                    for (int row=rowStart; row<rowEnd; row += increment)
                    {
                        ...
                        (*positions)[static_cast<unsigned int>(vertX*numVerts + vertY)]
                            = vsg::vec3((vertX / float(numVerts - 1) - 0.5f) * size * Constants::CellSizeInUnits,
                        ...
                        if (colourData)
                        {
                            color = {colourData->mColours[srcArrayIndex],
                                colourData->mColours[srcArrayIndex+1],
    ...
    void Storage::getBlendmaps(float chunkSize, const vsg::vec2 &chunkCenter, DataList &blendmaps, std::vector<Terrain::LayerInfo> &layerList)
    {
        ...
        for (int y=0; y<blendmapSize; y++)
        {
            for (int x=0; x<blendmapSize; x++)
            {
                ...
                if (found == textureIndicesMap.end())
                {
                    ...
                    {
                        auto image = vsg::ubyteArray::create(blendmapImageSize, blendmapImageSize, vsg::Data::Layout{.format=VK_FORMAT_R8_UNORM});
                        blendmaps.emplace_back(image);
                        layerList.emplace_back(info);
                    }
                }
                unsigned int layerIndex = found->second;
                unsigned char* pData = reinterpret_cast<unsigned char*>(blendmaps[layerIndex]->dataPointer());
