...
namespace
{
    ...
    {
        UpdateEquipment(MWAnim::Wielding *w) : actor(w) {}
...
bool CharacterController::onOpen() const
{
    if (mPtr.getType() == ESM::Container::sRecordId)
    {
        ...
        mAnimation->play("containeropen", Priority_Persistent, MWAnim::BlendMask_All, false, 1.0f, "start", "stop", 0.f, 0);
        if (mAnimation->isPlaying("containeropen"))
            return false;
    }
...
void CharacterController::onClose() const
{
    if (mPtr.getType() == ESM::Container::sRecordId)
    {
        ...
        mAnimation->play("containerclose", Priority_Persistent, MWAnim::BlendMask_All, false, 1.0f, "start", "stop", startPoint, 0);
    }
}
...
std::string CharacterController::fallbackShortWeaponGroup(const std::string& baseGroupName, MWAnim::BlendMask* blendMask) const
{
...
void CharacterController::refreshIdleAnims(CharacterState idle, bool force)
{
    ...
    mAnimation->play(mCurrentIdle, priority, MWAnim::BlendMask_All, false, 1.0f, "start", "stop", startPoint, numLoops, true);
}
...
void CharacterController::playDeath(float startpoint, CharacterState death)
{
    ...
    mAnimation->play(mCurrentDeath, Priority_Death, MWAnim::BlendMask_All,
...
{
    {
        ...
        mAnimation->tagHandler = std::bind(&CharacterController::handleTextKey, this, _1, _2, _3);
    }
    ...
    if (mWielding)
        mPtr.getClass().getInventoryStore(mPtr).setInvListener(new UpdateEquipment(mWielding), mPtr);
...
CharacterController::~CharacterController()
{
    ...
    if (mWielding)
    {
        ...
        mPtr.getClass().getInventoryStore(mPtr).setInvListener(nullptr, mPtr);
    }
    mAnimation->tagHandler = {};
    persistAnimationState();
}

void CharacterController::handleTextKey(std::string_view groupname, Anim::Tags::ConstIterator key, const Anim::Tags& map)
{
...
bool CharacterController::updateCarriedLeftVisible(const int weaptype) const
{
    ...
    return 0;//mAnimation->updateCarriedLeftVisible(weaptype);
}
...
bool CharacterController::updateWeaponState()
{
            ...
            if(mWeaponType == ESM::Weapon::Spell)
            {
                ...
                else if (!spellid.empty() && spellCastResult != MWWorld::SpellCastState::PowerAlreadyUsed)
                {
                    ...
                    if (mCanCast)
                    {
                        ...
                        {
                            MWRender::addEffect(mPtr, "VFX_Hands", -1, false, "Bip01 L Hand", effect->mParticle);
                            MWRender::addEffect(mPtr, "VFX_Hands", -1, false, "Bip01 R Hand", effect->mParticle);
                        }
                    }
    ...
    if (mUpperBodyState == UpperBodyState::AttackRelease)
    {
        ...
        if (!animPlaying || (currentTime >= maxAttackTime && complete >= 1.f))
        {
            ...
            mAnimation->play(mCurrentWeapon, priorityFollow, MWAnim::BlendMask_All, false, weapSpeed, mAttackType + ' ' + start, mAttackType + ' ' + stop, 0.0f, 0);
            mUpperBodyState = UpperBodyState::AttackEnd;
...
void CharacterController::update(float duration)
{
    ...
    auto moved = mAnimation->runAnimation(mSkipAnim && !isPersist ? 0.f : duration);
    if (isPersist)
    ...
    if (mActor)
    {
        mActor->manualAnimation(mPtr.getRefData().getPosition().rot[0]);
        ...
        if (mWielding)
        {
            ...
            if (mNpc)
            {
                ...
                mNpc->setFacialState(!cls.getCreatureStats(mPtr).isDead(), soundMgr->sayActive(mPtr), soundMgr->getSaySoundLoudness(mPtr));
                if (mPlayer)
                {
                ...
                for (auto &soundId : mNpc->soundsToPlay)
                {
                    if (mPlayingSounds.find(soundId) == mPlayingSounds.end())
                        mPlayingSounds[soundId] = soundMgr->playSound3D(mPtr, soundId, 1.0f, 1.0f, MWSound::Type::Sfx, MWSound::PlayMode::Loop);
                }
            }
        }
    }
...
void CharacterController::unpersistAnimationState()
{
    ...
    if (!state.mScriptedAnims.empty())
    {
        ...
        mAnimation->play(anim.mGroup,
                         Priority_Persistent, MWAnim::BlendMask_All, false, 1.0f,
...
bool CharacterController::playGroup(const std::string &groupname, int mode, int count, bool persist)
{
    ...
    if(mode != 0 || mAnimQueue.empty() || !isAnimPlaying(mAnimQueue.front().mGroup))
    {
        ...
        mAnimation->play(groupname, persist && groupname != "idle" ? Priority_Persistent : Priority_Default,
                            MWAnim::BlendMask_All, false, 1.0f,
...
void CharacterController::updateMagicEffects() const
{
    ...
    float light = magicEffects.get(ESM::MagicEffect::Light).getMagnitude() * std::ceil(Constants::UnitsPerFoot) * /*arbitraryMultiplierToMakeCutoffLessObvious*/3;
...
void CharacterController::updateHeadTracking(float duration)
{
    ...
    if (!mHeadTrackTarget.isEmpty())
    {
        ...
        if (auto *anim = MWBase::Environment::get().getWorld()->getAnimation(mHeadTrackTarget))
        {
            ...
            if (!node.empty())
                direction = vsgUtil::computePosition(node) - vsgUtil::computePosition(mActor->worldTransform(*head));
            else
            {
                // no head node to look at, fall back to look at center of collision box
                direction = toVsg(MWBase::Environment::get().getWorld()->aimToTarget(mPtr, mHeadTrackTarget, false));
            }
        }
    ...
    /*
    void addSpellCastGlow(const ESM::MagicEffect *effect, float glowDuration)
    {
