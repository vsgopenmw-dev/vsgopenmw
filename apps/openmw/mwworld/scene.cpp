...
namespace
{
    struct UpdateCache : public vsg::Operation
    {
        Resource::ResourceSystem* mResourceSystem{};
        UpdateCache(Resource::ResourceSystem* resourceSystem) : mResourceSystem(resourceSystem) {}
        void run() override { mResourceSystem->updateCache(0); }
    };
...
namespace MWWorld
{
    ...
    void Scene::changePlayerCell(CellStore* cell)
    {
        ...
        mRendering.setRenderModeActive(MWRender::Render_Terrain, cell->isExterior());
        ...
        mRendering.getOperationThreads()->add(vsg::ref_ptr{ new UpdateCache(mRendering.getResourceSystem()) });
    }
    ...
    {
        mPreloader = std::make_unique<CellPreloader>(rendering.getAnimContext(), mPhysics->actorReadOptions(), mPhysics->readOptions(),
            rendering.getTerrain(), rendering.getLandManager(), rendering.getOperationThreads());
    ...
    void Scene::changeToExteriorCell(const ESM::Position& position, bool changeEvent)
    {
    ...
    */
    ...
    float /*progress*/  Scene::preloadCell(const ESM::CellId& cell, bool preloadSurrounding)
    {
        return preloadCell(mWorld.getWorldModel().getCell(cell), preloadSurrounding);
    }
    ...
    float /*progress*/ Scene::preloadCell(CellStore* cell, bool preloadSurrounding)
    {
        ...
        if (preloadSurrounding && cell->isExterior())
        {
            ...
            for (int dx = -mHalfGridSize; dx <= mHalfGridSize; ++dx)
            {
                ...
                {
                    auto cell = mWorld.getWorldModel().getExterior(x + dx, y + dy);
                    if (mActiveCells.find(cell) != mActiveCells.end())
                        continue;
