...
#include <components/terrain/group.hpp>
...
namespace MWWorld
{
    ...
    public:
        ...
        PreloadCell(MWWorld::CellStore* cell, const MWAnim::Context& animContext, vsg::ref_ptr<const vsg::Options> actorShapeOptions,
            ...
            Terrain::Group* terrain, MWRender::LandManager* landManager)
        ...
        void operate() override
        {
            if (mIsExterior)
            {
                try
                {
                    if (auto terrainOp = mTerrain->cacheCell(mX, mY))
                    {
                        if (compileThreads)
                            compileThreads->add(terrainOp);
                        else
                            terrainOp->run();
                        mPreloadedObjects.insert(terrainOp);
                    }
    ...
    CellPreloader::CellPreloader(const MWAnim::Context& animContext, vsg::ref_ptr<const vsg::Options> actorShapeOptions,
        vsg::ref_ptr<const vsg::Options> shapeOptions,
        Terrain::Group* terrain, MWRender::LandManager* landManager, vsg::ref_ptr<vsg::OperationThreads> threads)
        : mAnimContext(animContext)
        , mActorShapeOptions(actorShapeOptions)
        , mShapeOptions(shapeOptions)
        , mTerrain(terrain)
    ...
    float CellPreloader::preload(CellStore* cell)
    {
        ...
        vsg::ref_ptr<PreloadCell> item(new PreloadCell(cell, mAnimContext, mActorShapeOptions, mShapeOptions, mTerrain, mLandManager));
