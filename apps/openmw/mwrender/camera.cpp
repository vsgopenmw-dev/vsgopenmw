...
namespace MWRender
{
    ...
    void Camera::updateCamera(vsg::LookAt& lookAt)
    {
        auto orient = vsg::quat(mRoll + mExtraRoll, vsg::vec3(0, 1, 0))
            * vsg::quat(mPitch + mExtraPitch, vsg::vec3(1, 0, 0)) * vsg::quat(mYaw + mExtraYaw, vsg::vec3(0, 0, 1));
        ...
        mViewMatrix = toOsg(vsg::mat4(lookAt.transform()));
    }
    ...
    void Camera::setMode(Mode newMode, bool force)
    {
        ...
        Mode oldMode = mMode;
        if (!force && (newMode == Mode::FirstPerson || oldMode == Mode::FirstPerson)
            && !(mAnimation && mAnimation->busy))
        {
