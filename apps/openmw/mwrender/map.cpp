...
namespace MWRender
{
    ...
    public:
        ...
        Blit(int32_t resolution, vsg::ref_ptr<vsg::Image> srcImage, vsg::ref_ptr<vsg::View> view)
            : mView(view)
        {
            VkImageBlit region{};
        ...
        void accept(vsg::RecordTraversal& visitor) const override
        {
            ...
            for (auto& r : requests)
            {
                ...
                for (auto& sc : r.descriptorSet->state->stateCommands)
                    sc->accept(visitor);
                ...
                availableViewData.push_back(r.descriptorSet);
            }
            requests.clear();
        }
    };
    ...
    Map::Map(vsg::Context& ctx, vsg::ref_ptr<vsgUtil::CompileContext> compile, vsg::ref_ptr<vsg::Node> scene, uint32_t resolution, const vsg::Options* shaderOptions)
        : mContext(&ctx)
        , mSceneGraph(scene)
        ...
        , mCompile(compile)
    {
        ...
        auto view = vsgUtil::createSharedView(camera, scene);
        // auto lightGrid = std::make_unique<View::LightGrid>(shaderOptions);
        // setResolution()
        view->mask = ~Mask_Particle;
        view->viewDependentState = collectLights;
        view->bins.push_back(vsg::Bin::create(Bin_DepthSorted, vsg::Bin::DESCENDING));

        vsg::ref_ptr<vsg::ImageView> srcImageView;
        auto renderGraph = Render::createRenderTexture(ctx, VkExtent2D{ resolution, resolution }, srcImageView,
            VK_IMAGE_USAGE_TRANSFER_SRC_BIT, VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL);
        renderGraph->setClearValues(VkClearColorValue{ { 0, 0, 0, 1 } });
        renderGraph->children = { view };

        mBlit = new Blit(resolution, srcImageView->image, view);
        ...
        int segment_x, int segment_y, float x, float y, const vsg::dvec3& upVector, float zmin, float zmax)
    {
        ...
        auto descriptorSet = getOrCreateViewData();
        if (!descriptorSet)
            return;
        descriptorSet->sceneData->value().invView = vsg::mat4(vsg::inverse_4x3(request.viewMatrix->transform()));
        descriptorSet->sceneData->setLightPosition({ -0.3, -0.3, 0.7 }, descriptorSet->sceneData->value().invView);
        descriptorSet->sceneData->setDepthRange(
    ...
    Map::Texture* Map::getOrCreateTexture()
    {
        ...
        dstImage->format = Render::compatibleColorFormat;
