...
namespace
{
    std::string getModel(const ESM::RefId& bodyPart, const Store& store)
    {
    ...
    std::string getVampireHead(const ESM::RefId& race, bool female, const Store& store)
    {
    ...
    {
        auto str = bodypart->mId.getRefIdString();
        return str.size() >= 3 && str.substr(str.size() - 3, 3) == "1st";
    }
...
namespace MWRender
{
    ...
    const std::vector<const ESM::BodyPart*>& getBodyParts(
        const ESM::RefId& race, bool female, bool firstPerson, bool werewolf, const Store& store)
    {
    ...
    std::string getHeadModel(bool werewolf, bool vampire, bool female, const ESM::RefId& race,
        const ESM::RefId& bodypartName, const Store& store)
    {
        if (vampire && !werewolf)
        {
            ...
            return getModel(ESM::RefId::stringRefId("WerewolfHead"), store);
        if (!bodypartName.empty())
            return getModel(bodypartName, store);
        return {};
    }

    std::string getHairModel(bool werewolf, const ESM::RefId& hairName, const Store& store)
    {
        if (werewolf)
            return getModel(ESM::RefId::stringRefId("WerewolfHair"), store);
        if (!hairName.empty())
            return getModel(hairName, store);
        return {};
    }

    const ESM::BodyPart* getBodyPart(bool firstPerson, const ESM::RefId& base, const Store& store)
    {
        const char* ext = firstPerson ? ".1st" : "";
        auto bodypart = store.search(ESM::RefId::stringRefId(base.getRefIdString() + ext));
        if (!bodypart && firstPerson)
        {
            bodypart = store.search(base);
            if (bodypart
                && !(bodypart->mData.mPart == ESM::BodyPart::MP_Hand || bodypart->mData.mPart == ESM::BodyPart::MP_Wrist
                    || bodypart->mData.mPart == ESM::BodyPart::MP_Forearm
                    || bodypart->mData.mPart == ESM::BodyPart::MP_Upperarm))
                bodypart = nullptr;
        }
