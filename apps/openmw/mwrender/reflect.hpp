...
namespace MWRender
{
    ...
    public:
        ...
        {
            uint32_t res = 512;
            mDstColorImage = vsg::Image::create();
            mDstColorImage->extent = { res, res, 1 };
            mDstColorImage->arrayLayers = 1;
            mDstColorImage->mipLevels = 1;
            mDstColorImage->usage = VK_IMAGE_USAGE_SAMPLED_BIT | VK_IMAGE_USAGE_TRANSFER_DST_BIT;
            mDstColorImage->format = Render::compatibleColorFormat;

            mDstDepthImage = vsg::Image::create();
            mDstDepthImage->extent = { res, res, 1 };
            mDstDepthImage->arrayLayers = 1;
            mDstDepthImage->mipLevels = 1;
            mDstDepthImage->usage = VK_IMAGE_USAGE_SAMPLED_BIT | VK_IMAGE_USAGE_TRANSFER_DST_BIT;
            mDstDepthImage->format = Render::compatibleDepthFormat;

            auto initialColorLayout = VK_IMAGE_LAYOUT_PRESENT_SRC_KHR;
            ...
            // region.srcOffsets[1] =
            region.dstOffsets[1] = VkOffset3D{ static_cast<int32_t>(res), static_cast<int32_t>(res), 1 };
            // mBlitColor->srcImage =
