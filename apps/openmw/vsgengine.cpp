...
namespace
{
    ...
    vsg::ref_ptr<vsg::Sampler> createDefaultSampler(vsg::ref_ptr<vsg::PhysicalDevice> physicalDevice, bool supportsAnisotropy)
    {
        ...
        sampler->maxAnisotropy = std::max(1, std::min(Settings::Manager::getInt("anisotropy", "General"), static_cast<int>(physicalDevice->getProperties().limits.maxSamplerAnisotropy)));
        ...
        sampler->minFilter = getFilterMode(Settings::Manager::getString("texture min filter", "General"));
        sampler->magFilter = getFilterMode(Settings::Manager::getString("texture mag filter", "General"));
